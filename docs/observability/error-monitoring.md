...
Not every error is a bug. An exception your code throws on purpose, such as a `NotFoundException` or a failed validation, still shows up in the **Errors** view, since it is still a failed request, but it doesn't count as a new defect for alerting. Only unhandled failures do: an error whose request answered with a 5xx, or one that failed on an entry point with no HTTP status at all, such as an RPC handler or a gateway message.

#### Source context

A stack trace from production usually points at compiled output you don't have checked out. With `sourceContext` enabled (the default), the SDK reads the source lines around each in-app frame of a captured error and sends them with it, so the dashboard shows the failing code next to the stack trace:
...
**Introduced in** comes from the `serviceVersion` your application reports (a semantic version or a commit hash, passed to [`forRoot()`](/observability/sdk)). Without it, a group can't tell which deploy it arrived with.

The two closed states are deliberately asymmetric. **Resolved** is a claim about the code, so a resolved group reopens as soon as the error recurs; a fix that didn't hold surfaces instead of staying quietly green. **Ignored** is a decision about noise, and stays ignored.
...
- A **new error groups** rule fires when a defect appears for the first time, for one application or the whole project. It notifies through email, Slack, a webhook, or an in-app notification, or by opening an issue.
...
Because each error already carries its stack trace, source lines, trace, and logs, **Copy agent prompt** on its page packages all of it into a self-contained prompt for a coding agent such as Claude Code, Cursor, or any other tool that has your repository open. An agent connected to the [MCP server](/observability/mcp-server) can go further on its own: list error groups with their introducing release, pull recent occurrences with their trace ids, tell whether an error is growing or decaying, and see which users it affected.
